...
DEFINE_string(image_dir, "", "image dir containing images");
DEFINE_string(pose_file, "", "image pose file");
DEFINE_int32(width, 640, "image width");
DEFINE_int32(height, 480, "image height");
DEFINE_string(image_ext, ".png", "image extention");
...
void showUsage(char **argv) {
    std::cout << "usage: " << argv[0] << " image_dir pose_file width height ext"
              << std::endl;
}
...
//    demo(VOC_FILE, IMAGE_DIR, POSE_FILE, IMAGE_W, IMAGE_H);
  demoDetector<BriefVocabulary, BriefLoopDetector, FBrief::TDescriptor>
    demo(VOC_FILE, FLAGS_image_dir, FLAGS_pose_file, FLAGS_width, FLAGS_height,
         FLAGS_image_ext, FLAGS_result_file);
